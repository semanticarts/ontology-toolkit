...
def test_filter_bnode_subjects():
    onto_tool.main([
    ...
    assert not os.path.isfile('tests-output/graphic/test_filter_bnode_subjects.dot')


def test_show_bnode_subjects():
    onto_tool.main([
        'graphic', '--predicate-threshold', '0', '--data',
        '--show-bnode-subjects',
        '-t', 'Blank Node Subjects Shown',
        '--no-image',
        '-o', 'tests-output/graphic/test_show_bnode_subjects',
        'tests/graphic/issue-116-bnode-subjects.ttl',
        'tests/graphic/issue-116-ontology.ttl'
    ])
    (instance_graph,) = pydot.graph_from_dot_file('tests-output/graphic/test_show_bnode_subjects.dot')
...
def test_show_named_subjects():
    onto_tool.main([
        'graphic', '--predicate-threshold', '0', '--data',
        '--show-bnode-subjects',
        '-t', 'Named Node Subjects',
        '--no-image',
        '-o', 'tests-output/graphic/test_named_subjects',
        'tests/graphic/issue-116-named-subjects.ttl',
    ])
    (instance_graph,) = pydot.graph_from_dot_file('tests-output/graphic/test_named_subjects.dot')
    edges = list(sorted((e.get_source(), e.get_destination()) for e in instance_graph.get_edges()))
    assert edges == [('"http://example.org/Person"', '"http://example.org/State"')]


def test_show_ont_bnode_subjects():
    onto_tool.main([
    ...
    (instance_graph,) = pydot.graph_from_dot_file('tests-output/graphic/test_ont_bnode_subjects.dot')
